...
- FROM: `### Requirement: Proactive Discord posting is channel-only in this change`
- TO: `### Requirement: Proactive Discord posting targets channels and direct messages`

## MODIFIED Requirements

### Requirement: Agent-initiated proactive Discord channel post

The system SHALL support agent-initiated proactive Discord posts through the
generic `send_channel_message` LLM tool with `channel_key = "discord"`. The
per-channel `send_discord_message` tool is removed; proactive posting SHALL be
dispatched through the Discord `IChannelOutboundClient` registered by the
channel integration.

The tool SHALL require an explicit resolved `destination` (a stable channel id
from `lookup_channel_destination`, or a stable user id from
`lookup_channel_user` for direct messages). There SHALL be no implicit
default-channel fallback at the tool layer.

The Discord outbound client SHALL enforce the Discord channel ACL
(`DiscordAclPolicy.IsAllowedChannel`, which permits the default channel or any
entry in `AllowedChannelIds`) before posting. A destination that is not allowed
SHALL be rejected with an error and SHALL NOT be posted to.

For channel destinations, the client SHALL post the message and create a public
thread off the posted message (fixed thread name "Conversation"), such that the
posted message is the thread root. The new session identity SHALL be
`{parentChannelId}/{threadId}`, consistent with how inbound thread messages
resolve their session identity.

#### Scenario: Proactive post to an allowed channel creates a thread

- **GIVEN** a Discord channel id present in `AllowedChannelIds`
- **WHEN** the agent calls `send_channel_message` with `channel_key = "discord"`
  and a destination resolved to that channel
...
- **GIVEN** a Discord channel id that is neither the default channel nor in
  `AllowedChannelIds`
- **WHEN** the agent calls `send_channel_message` for that destination
...
- **WHEN** the agent calls `send_channel_message` with an empty or
...
- **GIVEN** a thread created by a proactive `send_channel_message` call
...
- **GIVEN** a thread created by a proactive `send_channel_message` call, whose root is the bot's
  posted message
...
### Requirement: Proactive Discord posting targets channels and direct messages

Proactive Discord posting through `send_channel_message` SHALL target Discord
channel destinations (`destination.kind = "destination"`) or direct messages
(`destination.kind = "direct_message"`). Direct messages SHALL be permitted only
when direct messages are enabled in channel configuration, and the target user
SHALL be validated against the configured user ACL
(`DiscordAclPolicy.IsAllowedUser`); ephemeral DM channel ids SHALL NOT be
required in the channel allowlist.

#### Scenario: Proactive direct message to an allowed user

- **GIVEN** Discord direct messages are enabled and a user id passes the user ACL
- **WHEN** the agent calls `send_channel_message` with
  `destination.kind = "direct_message"` for that user
- **THEN** the message is delivered as a direct message

#### Scenario: Proactive direct message blocked when DMs are disabled

- **GIVEN** Discord channel configuration has direct messages disabled
- **WHEN** the agent calls `send_channel_message` with
  `destination.kind = "direct_message"`
